...
add_action(
	...
	function( $action = '', $plugin_name = '' ) {
		...
		if ( ! is_wp_error( $response ) && isset( $response['body'] ) && ( json_decode( $response['body'] ) !== null ) ) {
			...
			set_transient( VDZ_CU_API . '_activation_notice_text', $response['body'], 60 );
		}
	},
	10,
	2
...
add_action(
	...
	function () {
		...
		$message     = get_transient( VDZ_CU_API . '_activation_notice_text' );
		...
		if ( ! empty( $all_plugins ) ) {
			...
			printf( '<div id="' . esc_attr( VDZ_CU_API ) . '" class="%1$s">%2$s</div>', esc_attr( $class ), wp_kses_post( $msg ) );
			// Мгновенное удаление временного сообщения
			delete_transient( VDZ_CU_API . '_activation_notice_text' );
		}
	}
);
